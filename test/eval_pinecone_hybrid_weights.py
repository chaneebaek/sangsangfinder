"""
Evaluate production-like Pinecone hybrid search across keyword/vector weights.

This script intentionally uses api.services.search_service.hybrid_search instead
of rebuilding dense scores in memory, so the evaluation follows the same Pinecone
...
Ground truth:
  ...
  - Matching is done by notice URL, not title, because Pinecone stores chunked
...
def main() -> None:
    ...
    print("=" * 78)
    print("Production-like Pinecone hybrid weight evaluation")
    print("=" * 78)
    ...
    if present < total:
        ...
        if not args.allow_missing_gt:
            print("Abort: re-index the target corpus into Pinecone or pass --allow-missing-gt to continue anyway.")
            return
